            ...
            <div class="fx1 overflow-overlay d-flex flex-column">
                <v-container class="overflow-overlay" height="700">
                    <v-card flat>
                        ...
                        <v-card-text class="overflow-overlay h500" v-if="common.isEmpty(menuList)">
                          그룹 목록에서 그룹을 선택해주세요.
                        </v-card-text>
                        <v-card-text class="overflow-overlay h500" v-if="!common.isEmpty(menuList)">
                            <!-- openOnClick-->
                            <v-treeview
                            v-model:selected="filteredMenuList"
                            :items="menuList"
                            item-value="menuId"
                            item-title="menuNm"
...
const save = async() => {
  let params = {
    ...
    "menuIds":filteredMenuList.value
  };

  await common.sendByTrnCd('MNM11I01', params, (req, res)=> {
    common.showSnackbar(`메뉴권한을 등록했습니다.`, "primary", 2000);
  }, (params, res)=>{
      common.showSnackbar(`메뉴권한 등록 중 오류가 발생되었습니다.`, "red", 2000);
  });
};
...
watch( menuList, (newItem, oldItem) => {
